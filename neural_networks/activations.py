...
class SoftMax(Activation):
    ...
    def forward(self, Z: np.ndarray) -> np.ndarray:
        ...
        return np.exp(Z - maxes)/np.sum(np.exp(Z - maxes), axis = 1, keepdims=True)
    ...
    def backward(self, Z: np.ndarray, dY: np.ndarray) -> np.ndarray:
        ...
        for i in range(len(sigma)):
            d = np.diagflat(sigma[i])
            sigma_reshaped = sigma[i].reshape((-1,1))
            t = d - np.dot(sigma_reshaped, sigma_reshaped.T)
            ans.append(dY[i]@t)
